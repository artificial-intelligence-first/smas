...
def _detect_orphans(graph: Dict[str, Set[str]], logger: ObservabilityLogger) -> List[str]:
  ...
  orphans = [path for path in all_files - referenced if not path.endswith("README.md")]

  logger.log("orphans_detected", {"count": len(orphans)})
  return orphans


def _detect_cycles(graph: Dict[str, Set[str]], logger: ObservabilityLogger) -> List[List[str]]:
