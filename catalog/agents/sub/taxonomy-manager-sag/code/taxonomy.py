...
def _find_similar_terms(term: str, controlled_terms: Set[str]) -> List[str]:
    ...
    similar = [
        candidate for candidate in controlled_terms if candidate.startswith(term[:3])
    ]
    return similar[:3]
...
def _generate_recommendations(
    usage: Dict[str, int], controlled_terms: Set[str]
) -> List[str]:
    ...
    recommendations.append(
        "Frequent terms: " + ", ".join(f"{term}({count})" for term, count in frequent)
    )
